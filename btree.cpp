...
BTree::BTree(const std::string &filename) : root(nullptr), t(0) {
  if (!build_tree(filename)) {
    std::cerr << "Error in building BTree from file. Check formatting in "
              << filename << "\n";
  }
}
...
void recursive_destructor(Node *node) {
  ...
  for (int i = 0; i < node->n + 1; i++) {
    ...
    if (next_child == nullptr) {
      ...
      recursive_destructor(next_child);
    }
  }
...
bool BTree::build_tree(const std::string &filename) {
  ...
  if (levels.empty()) {
    ...
    for (size_t i = 0; i < levels.size() - 1;
         i++) // don't need to link leaf nodes to any children
    {
      int child_index = 0;
